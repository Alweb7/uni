...
/* Estrae un indice casuale valido all'interno del menu richiesto. */
static int choose_random_menu_index(int count) {
    return random_in_range(0, count - 1);
}

/* Sceglie la prossima stazione tra quelle ancora desiderate guardando la lunghezza delle code. */
...
/* Azzera lo stato giornaliero dell'utente prima dell'avvio di una nuova giornata. */
...
/* Attende la risposta di una stazione fino a fine giornata o shutdown. */
...
/* Invia la richiesta a una stazione e attende l'esito del servizio. */
...
/* Registra che un membro del gruppo parteciperà alla giornata corrente. */
static void group_arrive(shared_state_t *shared, int group_id) {
    /* group punta allo stato condiviso del gruppo dell'utente corrente. */
    group_state_t *group = &shared->groups[group_id];
    /* planned_today cresce solo per i membri che oggi partecipano davvero alla mensa. */
    semaphore_wait(&group->mutex);
    group->planned_today += 1;
    semaphore_post(&group->mutex);
}

/* Sincronizza i membri presenti del gruppo prima del passaggio finale alla cassa. */
static bool group_wait_ready(shared_state_t *shared, int group_id) {
    /* releases indica quanti sem_post fare quando l'ultimo membro arriva alla barriera. */
    int releases = 0;
    /* group è il record di sincronizzazione del gruppo di appartenenza. */
    group_state_t *group = &shared->groups[group_id];
    long long timeout_ns = simulate_seconds_to_nanoseconds(&shared->cfg, 1);
    /* L'ultimo membro arrivato alla barriera sblocca tutti i partecipanti del gruppo. */
    semaphore_wait(&group->mutex);
    group->ready_today += 1;
...
/* Simula il passaggio dell'utente con ticket nella fase di lettura del ticket. */
static bool maybe_ticket(shared_state_t *shared, user_state_t *user) {
    long long timeout_ns = simulate_seconds_to_nanoseconds(&shared->cfg, 1);
    if (!user->has_ticket) {
        return true;
    }
    /* Il lettore ticket è una risorsa condivisa con capacità configurabile. */
    while (shared->day_open && !shared->shutdown) {
...
/* Attende un posto libero e simula il consumo del pasto ai tavoli della mensa. */
static void consume_meal_at_table(shared_state_t *shared, user_state_t *user) {
    ...
    long long timeout_ns = simulate_seconds_to_nanoseconds(&shared->cfg, 1);
    /* The table semaphore models the dining seats directly and avoids polling. */
    while (shared->day_open && !shared->shutdown) {
    ...
    /* Il tempo passato al tavolo cresce con il numero di piatti realmente acquistati. */
...
/* Implementa il comportamento quotidiano di un processo utente all'interno della simulazione. */
