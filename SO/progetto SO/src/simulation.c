...
static void accumulate_statistics(stats_snapshot_t *dst, const stats_snapshot_t *src) {
    /* i attraversa tutte le stazioni per accumulare gli array statistici. */
    int i;
...
static long total_wait_count(const stats_snapshot_t *stats) {
    /* total accumula il numero di campioni registrati su tutte le stazioni. */
    long total = 0;
...
/* Calcola la somma totale dei tempi di attesa su tutte le stazioni. */
static long long total_wait_ns(const stats_snapshot_t *stats) {
    /* total somma il tempo complessivo passato in coda da tutti gli utenti. */
    long long total = 0;
    for (int i = 0; i < STATION_COUNT; ++i) {
...
/* Restituisce il numero totale di piatti distribuiti nelle stazioni di servizio. */
...
/* Restituisce il totale degli avanzi accumulati nelle stazioni con inventario finito. */
...
/* Conta quanti worker hanno lavorato almeno una volta nell'intera simulazione. */
static long unique_active_workers(const shared_state_t *shared) {
    /* total conta quanti worker sono stati attivi almeno una volta nella simulazione. */
    long total = 0;
...
/* Converte il codice di terminazione nella stringa usata nei report. */
...
bool enqueue_user(station_queue_t *queue, int user_id) {
    /* ok indica se l'inserimento in coda è andato a buon fine. */
    bool ok = false;
    /* Producer side del bounded buffer: prima attendo uno slot, poi entro in sezione critica. */
...
void assign_workers(shared_state_t *shared) {
    /* i indica la prossima posizione libera nell'array worker_assignments. */
    int i = 0;
    /* Si parte garantendo almeno un worker per stazione. */
    /* weights contiene il "peso" corrente di ciascuna stazione per la ripartizione. */
    ...
    /* workers_per_station è il risultato della distribuzione dei worker. */
    int workers_per_station[STATION_COUNT] = {1, 1, 1, 1};
    /* remaining sono i worker ancora da collocare oltre al minimo garantito. */
    int remaining = shared->cfg.num_workers - STATION_COUNT;
    /* I worker rimanenti vengono distribuiti favorendo i servizi mediamente più lenti. */
    while (remaining > 0) {
        int max_idx = 0;
...
void setup_day(shared_state_t *shared) {
    /* Reset delle metriche giornaliere e apertura formale del nuovo giorno simulato. */
    ...
    /* Primi e secondi vengono ricaricati alla quantità media iniziale per ogni variante. */
    ...
    for (int i = 0; i < STATION_COUNT; ++i) {
        /* Le code ricominciano vuote a ogni giornata, semafori contatore inclusi. */
    ...
    for (int i = 0; i < MAX_GROUPS; ++i) {
        /* I contatori di sincronizzazione dei gruppi sono validi solo dentro la giornata. */
    ...
    /* Tutti i processi vivi vengono rilasciati dal manager all'inizio del giorno. */
...
void close_day(shared_state_t *shared) {
    /* Chiudere la giornata impedisce nuove interazioni e consente il consolidamento finale. */
    shared->day_open = 0;
    /* Gli utenti rimasti senza esito esplicito vengono conteggiati come non serviti. */
    semaphore_wait(&shared->stats_mutex);
    ...
    semaphore_post(&shared->stats_mutex);
    semaphore_wait(&shared->inventory.mutex);
    /* Gli avanzi si calcolano fotografando ciò che resta nel magazzino a fine giorno. */
    for (int i = 0; i < shared->menu.primi_count; ++i) {
    ...
    semaphore_post(&shared->inventory.mutex);
    semaphore_wait(&shared->stats_mutex);
    /* Le statistiche del giorno vengono riversate nel totale cumulativo. */
...
void refill_inventory_if_needed(shared_state_t *shared, long long elapsed_ns, long long *next_refill_ns) {
    ...
    /* Il refill aggiunge porzioni ma non supera mai il tetto massimo configurato. */
    semaphore_wait(&shared->inventory.mutex);
    for (int i = 0; i < shared->menu.primi_count; ++i) {
    ...
    semaphore_post(&shared->inventory.mutex);
    /* Il refill successivo viene pianificato con una lieve variabilità casuale. */
    *next_refill_ns += simulate_seconds_to_nanoseconds(
        &shared->cfg,
        add_random_variation(shared->cfg.avg_refill_time * 60, 20));
}

/* Calcola la media in millisecondi a partire da tempo totale e numero di campioni. */
static double avg_wait_ms(long long total_ns, long count) {
    /* total_ns è il tempo totale accumulato, count il numero di campioni usati per la media. */
    if (count == 0) {
        return 0.0;
    }
...
void print_day_report(FILE *out, const shared_state_t *shared, int day, bool final_report, termination_reason_t reason) {
    /* s punta allo snapshot giusto: giornaliero o cumulativo finale. */
    const stats_snapshot_t *s = final_report ? &shared->total_stats : &shared->day_stats;
    /* elapsed_days normalizza le medie rispetto ai giorni effettivamente trascorsi. */
    int elapsed_days = final_report && day > 0 ? day : 1;
    /* Variabili di appoggio usate per stampare indicatori aggregati. */
    ...
    /* Il report finale usa il cumulato completo, quello giornaliero usa lo snapshot del giorno. */
    if (final_report) {
