...
/* Registra che il worker ha preso possesso di una postazione operativa per la giornata. */
...
/* Aggiorna lo stato condiviso quando un worker smette di presidiare una stazione. */
...
/* Preleva dal magazzino il piatto richiesto o una valida alternativa dello stesso tipo. */
static int take_dish_from_inventory(shared_state_t *shared, station_type_t station, int preferred) {
    /* found conterrà l'indice del piatto effettivamente assegnato, oppure -1 se assente. */
    int found = -1;
    /* La scelta tenta prima il piatto desiderato e poi un fallback dello stesso reparto. */
...
/* Gestisce una pausa del worker senza lasciare la stazione completamente scoperta. */
static void maybe_pause(shared_state_t *shared, int worker_id, station_type_t station, int *pauses_done) {
    /* La pausa è probabilistica, limitata giornalmente e mai consentita se lascerebbe la stazione scoperta. */
    ...
    shared->stations[station].active_workers -= 1;
    semaphore_post(&shared->stats_mutex);
    /* Liberando la seat_sem un altro worker può subentrare durante la pausa. */
...
/* Eroga il servizio richiesto dall'utente e aggiorna le statistiche della stazione. */
static void serve_request(shared_state_t *shared, int user_id, station_type_t station) {
    /* user punta al record condiviso dell'utente che stiamo servendo. */
    user_state_t *user = &shared->users[user_id];
    /* wait_ns misura solo il tempo trascorso in coda prima dell'inizio del servizio. */
    long long wait_ns = get_time_ns() - user->request_enqueued_ns;
    /* service_seconds è la durata simulata del servizio corrente. */
    int service_seconds = 1;
    /* served_dish_index descrive quale piatto è stato consegnato; -1 significa fallimento. */
    int served_dish_index = -1;
    /* Ogni stazione applica il proprio tempo medio e la propria logica di servizio. */
    if (station == STATION_PRIMI) {
    ...
    /* Il tempo di servizio è simulato come attesa bloccante del processo worker. */
